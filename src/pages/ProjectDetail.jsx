    ...
    <div>
      ...
      <Tabs defaultValue="overview">
        <TabsList className="flex-wrap h-auto">
          ...
          <TabsTrigger value="tasks"><ListChecks className="h-3.5 w-3.5 mr-1.5" />Tasks{tabCount(tasks.length)}</TabsTrigger>
          <TabsTrigger value="files"><FileText className="h-3.5 w-3.5 mr-1.5" />Files{tabCount(files.length)}</TabsTrigger>
          <TabsTrigger value="activities"><ActivityIcon className="h-3.5 w-3.5 mr-1.5" />Activities{tabCount(activities.length)}</TabsTrigger>
          <TabsTrigger value="ai"><Sparkles className="h-3.5 w-3.5 mr-1.5" />AI Analysis{tabCount(insights.length)}</TabsTrigger>
          <TabsTrigger value="risks"><ShieldAlert className="h-3.5 w-3.5 mr-1.5" />Risks{tabCount(risks.length)}</TabsTrigger>
          <TabsTrigger value="decisions"><Gavel className="h-3.5 w-3.5 mr-1.5" />Decisions{tabCount(decisions.length)}</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="overview" className="mt-5">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="rounded-xl border border-slate-200 bg-white p-5">
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                  <Fact label="Owner">{owner ? owner.name : project.principalInvestigator || "Unassigned"}</Fact>
                  <Fact label="Status"><StatusBadge status={project.status} /></Fact>
                  <Fact label="Priority"><PriorityBadge priority={project.priority} /></Fact>
                  <Fact label="Target Date">{formatDate(project.endDate)}</Fact>
                  <Fact label="Start Date">{formatDate(project.startDate)}</Fact>
                  <Fact label="Funding">{project.fundingSource || "—"}</Fact>
                </div>
                <div className="mt-5 pt-5 border-t border-slate-100">
                  ...
                  <div className="grid grid-cols-3 gap-4 mt-4 text-center">
                    <div className="rounded-lg bg-slate-50 py-2"><div className="text-lg font-semibold text-slate-800">{completedCount}</div><div className="text-xs text-slate-400">Completed</div></div>
                    <div className="rounded-lg bg-slate-50 py-2"><div className="text-lg font-semibold text-slate-800">{openCount}</div><div className="text-xs text-slate-400">Open</div></div>
                    <div className="rounded-lg bg-rose-50 py-2"><div className="text-lg font-semibold text-rose-700">{blockedCount}</div><div className="text-xs text-rose-400">Blocked</div></div>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-xl border border-slate-200 bg-white p-5">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="flex items-center gap-2 text-sm font-semibold text-slate-800"><Sparkles className="h-4 w-4 text-blue-700" /> AI Summary</h3>
                  <Button size="sm" variant="outline" onClick={generateAnalysis} disabled={generating}>
                    <Wand2 className="h-3.5 w-3.5 mr-1.5" /> {generating ? "Analyzing…" : latestInsight ? "Regenerate" : "Generate"}
                  </Button>
                </div>
                {latestInsight ? (
                  <div>
                    <div className="flex items-center gap-2 mb-2">
                      <span className={`text-[11px] font-medium rounded-full px-2 py-0.5 ${SEVERITY_COLORS[latestInsight.severity] || SEVERITY_COLORS.Low}`}>{latestInsight.severity}</span>
                      <span className="text-xs text-slate-400">Confidence {Math.round((latestInsight.confidence || 0) * 100)}% · {formatDate(latestInsight.created_date)}</span>
                    </div>
                    <p className="text-sm text-slate-700">{latestInsight.summary}</p>
                  </div>
                ) : (
                  <p className="text-sm text-slate-400">No AI analysis yet. Generate one to get an objective summary of progress, risks, and recommendations.</p>
                )}
              </div>
              ...
              <div className="rounded-xl border border-slate-200 bg-white">
                ...
                {activities.length === 0 ? <div className="px-5 py-8 text-center text-sm text-slate-400">No activities logged.</div> : (
            ...
            <div className="rounded-xl border border-slate-200 bg-white">
              <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
                <h3 className="text-sm font-semibold text-slate-800">Recent Evidence</h3>
              </div>
              <div className="p-3">
                {evidenceFiles.length === 0 ? (
                  <div className="px-2 py-8 text-center text-sm text-slate-400">No evidence uploaded yet.</div>
                ) : (
                  evidenceFiles.slice(0, 6).map((f) => (
                    <div key={f.id} className="flex items-center gap-3 px-2 py-2.5">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="text-xs text-slate-400">{f.category || f.type} · {f.created_date ? relativeTime(f.created_date) : ""}</div>
                      </div>
                    </div>
                  ))
                )}
        ...
        <TabsContent value="tasks" className="mt-5">
          {tasks.length === 0 ? <div className="text-sm text-slate-400 py-12 text-center">No tasks yet.</div> : (
        ...
        <TabsContent value="files" className="mt-5">
          {files.length === 0 ? <div className="text-sm text-slate-400 py-12 text-center">No files uploaded.</div> : (
              ...
              {files.map((f) => (
                <div key={f.id} className="rounded-lg border border-slate-200 bg-white p-4 flex items-start gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-slate-400">{f.type}{f.isEvidence ? " · Evidence" : ""} · {f.created_date ? relativeTime(f.created_date) : ""}</div>
                  </div>
                </div>
              ))}
            </div>
        ...
        <TabsContent value="activities" className="mt-5">
          {activities.length === 0 ? <div className="text-sm text-slate-400 py-12 text-center">No activities logged.</div> : (
            <div className="rounded-xl border border-slate-200 bg-white divide-y divide-slate-100">
              {activities.map((a) => {
                const m = memberById(a.memberId);
                return (
        ...
        <TabsContent value="ai" className="mt-5">
          <div className="flex items-center justify-between mb-4">
            <p className="text-sm text-slate-500">AI-generated insights for this project.</p>
            <Button onClick={generateAnalysis} disabled={generating}><Wand2 className="h-4 w-4 mr-1.5" /> {generating ? "Analyzing…" : "Generate Analysis"}</Button>
          </div>
          {insights.length === 0 ? (
            <div className="rounded-xl border border-dashed border-slate-200 bg-white py-16 text-center text-sm text-slate-400">No AI analysis yet. Generate one from this project's tasks, activities, and risks.</div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {insights.map((i) => (
                <div key={i.id} className="rounded-xl border border-slate-200 bg-white p-5">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-semibold text-blue-700 bg-blue-50 rounded-full px-2 py-0.5">{i.type}</span>
                  ...
                  <div className="text-xs text-slate-400 mt-3 pt-3 border-t border-slate-50">Confidence {Math.round((i.confidence || 0) * 100)}% · {formatDate(i.created_date)}</div>
                </div>
              ))}
            </div>
          )}
        ...
        <TabsContent value="risks" className="mt-5">
          <div className="flex items-center justify-end mb-4">
            <Button variant="outline" asChild><Link to="/risks"><ShieldAlert className="h-4 w-4 mr-1.5" /> Manage in Risk Register</Link></Button>
          </div>
          {risks.length === 0 ? <div className="text-sm text-slate-400 py-12 text-center">No risks identified for this project.</div> : (
            <div className="rounded-xl border border-slate-200 bg-white divide-y divide-slate-100">
              {risks.map((r) => (
                <div key={r.id} className="flex items-center gap-3 px-4 py-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {r.mitigation && <div className="text-xs text-slate-400 truncate">Mitigation: {r.mitigation}</div>}
                  </div>
        ...
        <TabsContent value="decisions" className="mt-5">
          <div className="flex items-center justify-end mb-4">
            <Button onClick={() => setDecOpen(true)}><Plus className="h-4 w-4 mr-1.5" /> Log Decision</Button>
          </div>
          {decisions.length === 0 ? <div className="text-sm text-slate-400 py-12 text-center">No decisions logged yet.</div> : (
              ...
              {decisions.map((d) => (
                <div key={d.id} className="rounded-xl border border-slate-200 bg-white p-5">
                  ...
                  <div className="flex items-center gap-3 mt-3 text-xs text-slate-400">
                    <span className="text-slate-500 font-medium">{d.decisionType}</span>
                    <span>·</span>
                    <span>Impact {d.impact}</span>
                    <span>·</span>
                    <span>By {memberName(d.decidedBy)}</span>
      ...
      <Dialog open={decOpen} onOpenChange={setDecOpen}>
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-3 py-2">
            <div><Label>Title</Label><Input value={decForm.title} onChange={(e) => setDecForm({ ...decForm, title: e.target.value })} placeholder="e.g. Switch to new deposition vendor" /></div>
            <div className="grid grid-cols-3 gap-3">
              <div>
                <Label>Type</Label>
                <Select value={decForm.decisionType} onValueChange={(v) => setDecForm({ ...decForm, decisionType: v })}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>{DECISION_TYPES.map((s) => <SelectItem key={s} value={s}>{s}</SelectItem>)}</SelectContent>
                </Select>
              </div>
              <div>
                <Label>Impact</Label>
              ...
              <div>
                ...
                <Select value={decForm.status} onValueChange={(v) => setDecForm({ ...decForm, status: v })}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>{["Proposed", "Approved", "Rejected", "Implemented", "Superseded"].map((s) => <SelectItem key={s} value={s}>{s}</SelectItem>)}</SelectContent>
                </Select>
              </div>
            </div>
            <div><Label>Rationale</Label><Textarea rows={3} value={decForm.rationale} onChange={(e) => setDecForm({ ...decForm, rationale: e.target.value })} /></div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setDecOpen(false)}>Cancel</Button>
            <Button onClick={addDecision} disabled={!decForm.title.trim()}>Log Decision</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
