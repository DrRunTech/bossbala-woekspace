...
export default function ClientDetail() {
  ...
  if (!client) {
    return <EmptyState icon={Briefcase} title="Client not found" description="This client may have been removed." action={<Button asChild><Link to="/clients">Back to Clients</Link></Button>} />;
  }
  ...
  return (
    <div>
      ...
      <PageHeader
        ...
        actions={<Button onClick={() => { setForm(emptyContract); setOpen(true); }}><Plus className="h-4 w-4 mr-1.5" /> New Contract</Button>}
      />

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="rounded-xl border border-slate-200 bg-white p-5 lg:col-span-1">
          <div className="flex items-center gap-2 mb-3">
            <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-slate-100 text-slate-600">{client.status}</span>
          </div>
          <div className="space-y-2 text-sm">
            {client.contactPerson && <div><div className="text-xs text-slate-400">Contact</div><div className="text-slate-700">{client.contactPerson}</div></div>}
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="grid grid-cols-3 gap-3">
            <div className="rounded-xl border border-slate-200 bg-white p-4"><div className="text-xs text-slate-400">Contracts</div><div className="text-lg font-semibold text-slate-900 mt-1">{contracts.length}</div></div>
            <div className="rounded-xl border border-slate-200 bg-white p-4"><div className="text-xs text-slate-400">Active</div><div className="text-lg font-semibold text-slate-900 mt-1">{activeCount}</div></div>
            <div className="rounded-xl border border-slate-200 bg-white p-4"><div className="text-xs text-slate-400">Total Value</div><div className="text-lg font-semibold text-slate-900 mt-1">{totalValue ? totalValue.toLocaleString() : "—"}</div></div>
          </div>

          <div className="rounded-xl border border-slate-200 bg-white">
            <div className="px-5 py-3 border-b border-slate-100 text-sm font-semibold text-slate-700">Contracts</div>
            {contracts.length === 0 ? (
              <EmptyState icon={FileText} title="No contracts yet" description="Add a contract to track value, timeline, and status." action={<Button onClick={() => setOpen(true)}><Plus className="h-4 w-4 mr-1.5" /> New Contract</Button>} />
            ) : (
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-3 py-2">
            <div><Label>Title</Label><Input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="e.g. Annual maintenance agreement" /></div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label>Type</Label>
                <Select value={form.type} onValueChange={(v) => setForm({ ...form, type: v })}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>{CONTRACT_TYPES.map((s) => <SelectItem key={s} value={s}>{s}</SelectItem>)}</SelectContent>
                </Select>
              </div>
              <div>
                <Label>Status</Label>
              ...
              <div><Label>Currency</Label><Input value="USD" disabled /></div>
          ...
          <DialogFooter>
            ...
            <Button onClick={save} disabled={saving || !form.title.trim()}>{saving ? "Creating…" : "Create Contract"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
