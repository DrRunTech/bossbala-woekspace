...
const TYPES = [
  ...
  { value: "project_vs_previous", label: "Current Project vs Previous Period" },
  { value: "member_vs_previous", label: "Current Member Activity vs Previous Period" },
];
...
export default function Comparison() {
  ...
  const run = async () => {
    ...
    try {
      ...
      setResult(res);
    } catch { setResult({ ok: false, error: "Comparison failed." });
    ...
    <div className="space-y-5">
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-5">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
          <div className="md:col-span-2">
            <Label className="mb-1.5 block">Comparison type</Label>
          ...
          <Button onClick={run} disabled={loading || (needsProject && !projectId) || (needsMember && !memberId)} className="w-full md:w-auto">
            {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <GitCompare className="h-4 w-4" />} Run comparison
          </Button>
        </div>
      </div>
      ...
      {loading && (
        <div className="bg-white rounded-xl border border-slate-200 p-10 flex items-center justify-center text-slate-400">
          <Loader2 className="h-5 w-5 animate-spin mr-2" /> Computing objective metrics…
        </div>
      )}
    ...
    <div className="space-y-5">
      <div className="bg-white rounded-xl border border-slate-200 p-5">
        <div className="flex items-center gap-2 text-xs text-slate-400 mb-1">
          ...
          <span className="ml-auto">Current: {period.current.start} → {period.current.end} · Previous: {period.previous.start} → {period.previous.end}</span>
        </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        <div className="px-5 py-3 border-b border-slate-100 text-sm font-semibold text-slate-800">Objective metrics</div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-5">
        <h3 className="text-sm font-semibold text-slate-800 mb-3 flex items-center gap-1.5"><Info className="h-4 w-4 text-slate-400" /> Limitations of the evidence</h3>
  ...
  return (
    <div className="bg-white p-4">
      <div className="flex items-center justify-between">
        ...
        {m.snapshot && <span className="text-[10px] uppercase tracking-wide text-slate-400 bg-slate-100 px-1.5 py-0.5 rounded">snapshot</span>}
      </div>
      <div className="flex items-end gap-3 mt-2">
        ...
        <div>
          <div className="text-[11px] text-slate-400">Previous</div>
